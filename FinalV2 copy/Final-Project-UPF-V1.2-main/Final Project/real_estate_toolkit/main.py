...
def main():
    ...
    raw_data = data_loader.load_data_from_csv("train.csv")
    ...
    # Step 2: Map dataset to House objects
    def map_to_houses(data):
        houses = []
        for row in data:
            try:
                house = House(
                    ...
                    bedrooms=int(row["bedroom_abv_gr"]),
                    year_built=int(row["year_built"]),
                    quality_score=QualityScore.GOOD  # Adjust logic to infer quality if needed
                )
                houses.append(house)
    ...
    housing_market_data = map_to_houses(cleaned_data)

    # Step 3: Initialize the simulation with dataset-driven houses
    simulation = Simulation(
        housing_market_data=housing_market_data,
        consumers_number=10,  # Adjust as needed
        years=5,
        annual_income=AnnualIncomeStatistics(minimum=30000, average=75000, standard_deviation=15000, maximum=150000),
        children_range=ChildrenRange(minimum=0, maximum=3),
        cleaning_market_mechanism=CleaningMarketMechanism.INCOME_ORDER_DESCENDANT
